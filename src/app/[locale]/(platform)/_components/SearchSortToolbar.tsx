  ...
  return (
    <div className="space-y-3 px-2 pt-2 sm:px-3">
      ...
      >
        <div
          className={cn(
            stackOnMobile ? 'flex min-w-0 items-center gap-2 sm:flex-1 sm:gap-3' : 'relative min-w-0 flex-1',
          )}
        >
          <SearchInput value={searchQuery} placeholder={searchPlaceholder} onChange={onSearchChange} />
  ...
  return (
    <Select items={items} value={value} onValueChange={(nextValue) => nextValue !== null && onValueChange(nextValue)}>
      <SelectTrigger
        ...
        className={cn(
          `w-9 justify-center gap-0 px-0 *:data-[slot=select-value]:hidden sm:w-fit sm:justify-between sm:gap-1.5 sm:px-2.5 sm:*:data-[slot=select-value]:flex dark:bg-transparent [&>svg:last-of-type]:hidden`,
        )}
      >
        {icon}
        <SelectValue />
